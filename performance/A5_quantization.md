...
- **MXFP8 and production FP4 (2026 datapoints).** MXFP8 (k=32, E8M0 scale) trains at ~BF16 parity with a simpler recipe than fine-grained FP8 — public proof point: **TorchTitan MXFP8 + DeepEP = +41% throughput on a DeepSeek-V3-style pretraining run on B200** (PyTorch blog, 2026). And FP4 is no longer research-only: **DeepSeek-V4 runs its MoE experts natively in FP4 (rest FP8)** (arXiv:2606.19348), OpenAI's **gpt-oss** ships **MXFP4 MoE** weights (that is how gpt-oss-120b fits one 80 GB GPU — Phase-2's S2 block), and NVIDIA publishes NVFP4 checkpoints of frontier models (GLM-5-NVFP4). The 2026 rule: **FP8 for training-critical tensors, FP4 for weight/serving tensors** — with §4's measurement discipline deciding which tensor is which.
...
- PyTorch, **TorchTitan MXFP8 + DeepEP on B200** (blog, 2026) — +41% on a DeepSeek-V3-style pretraining run.
- DeepSeek-AI, **DeepSeek-V4**, arXiv:2606.19348 (MoE experts FP4, rest FP8). · OpenAI, **gpt-oss** (MXFP4 MoE weights).
