...
Build: JIT via torch.utils.cpp_extension.load_inline, extra_cuda_cflags=['-arch=sm_120', ...].
"""
...
def _mod():  # noqa: ANN202 - opaque JIT module handle
    """JIT-compile (and cache) the extension. Blackwell sm_120; WMMA fp16 in / fp32|fp16 accumulate."""
    return load_inline(
        name="wmma_gemm_ext",
