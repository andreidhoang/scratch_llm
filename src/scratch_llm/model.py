...
# The KV-cache substrate lives in its own module (one-way dep: kv_cache never imports the
# architecture). Re-exported here so ``from scratch_llm.model import KVCache`` keeps working;
# the canonical import path is ``scratch_llm.kv_cache``.
from scratch_llm.kv_cache import (
    AnyKVCache,
    BatchedKVCache,
    ChunkPrefillView,
    KVCache,
