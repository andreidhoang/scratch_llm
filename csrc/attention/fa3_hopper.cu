...
// =============================================================================
// Builds the three CUtensorMap descriptors (Q, K, V — fp16, 128B-swizzled to
// match the kernel's SMEM layout), allocates O, and launches one CTA per
// (batch, head, q-tile). The kernel is warp-specialized (1 producer + N
// consumer warpgroups, ping-pong SMEM, TMA + WGMMA + mbarriers).
//
// Runtime correctness is honestly deferred to the H100/H200 rental day.
// =============================================================================
#ifdef TORCH_EXTENSION_NAME
#include <torch/extension.h>
#include <cuda.h>

torch::Tensor flash_attention_fa3_forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V,
                                          bool is_causal) {
    TORCH_CHECK(Q.is_cuda() && K.is_cuda() && V.is_cuda(),
                "flash_attention_fa3_forward: Q, K, V must be CUDA");
    TORCH_CHECK(Q.scalar_type() == at::kHalf && K.scalar_type() == at::kHalf
                && V.scalar_type() == at::kHalf,
                "flash_attention_fa3_forward: Q, K, V must be float16 (wgmma.f16 operands)");
    // Accept [..., S, D] and treat leading dims as flat batch; the kernel addresses one
    // (batch, head, q-tile) per CTA. Q,K,V must share the leading shape.
    TORCH_CHECK(Q.sizes() == K.sizes() && K.sizes() == V.sizes(),
                "flash_attention_fa3_forward: Q, K, V must have identical shape");
    TORCH_CHECK(Q.size(-1) == D, "flash_attention_fa3_forward: head dim must equal ", D);
    Q = Q.contiguous();
    K = K.contiguous();
    V = V.contiguous();
    const int total_rows = Q.numel() / D;
    const int seqlen_kv  = Q.size(-2);
    TORCH_CHECK(total_rows % Br == 0,
                "flash_attention_fa3_forward: (batch*head*seq) must be a multiple of ", Br);

    auto O = torch::empty_like(Q);
    const float softmax_scale = 1.0f / std::sqrt((float)D);

    // ---- Build three TMA descriptors (row-major 2D: (D=fast, row=slow)) ----
    // The kernel loads Q as (Br, D) and K/V as (Bc, D) tiles; the 128B swizzle
    // matches make_smem_desc's layout_type=1 in the kernel body.
    auto build_tmap = [](void* gmem_ptr, int nrows) {
        CUtensorMap tmap = {};
        const uint64_t size[2]    = {(uint64_t)D, (uint64_t)nrows};
        const uint64_t stride[1]  = {(uint64_t)D * sizeof(__half)};
        const uint32_t box[2]     = {(uint32_t)D, (uint32_t)Br};
        const uint32_t estride[2] = {1, 1};
        CUresult r = cuTensorMapEncodeTiled(
            &tmap, CU_TENSOR_MAP_DATA_TYPE_FLOAT16, 2, gmem_ptr, size, stride, box, estride,
            CU_TENSOR_MAP_INTERLEAVE_NONE, CU_TENSOR_MAP_SWIZZLE_128B,
            CU_TENSOR_MAP_L2_PROMOTION_NONE, CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
        TORCH_CHECK(r == CUDA_SUCCESS, "cuTensorMapEncodeTiled failed: ", (int)r);
        return tmap;
    };
    CUtensorMap tmap_Q = build_tmap(Q.data_ptr(), total_rows);
    CUtensorMap tmap_K = build_tmap(K.data_ptr(), total_rows);
    CUtensorMap tmap_V = build_tmap(V.data_ptr(), total_rows);

    // SMEM: Q tile + 2 ping-pong (K,V) stages + 5 mbarriers.
    const int smem_bytes = (Br * D + 2 * STAGES * Bc * D) * sizeof(__half) + 5 * sizeof(uint64_t) + 128;
    cudaFuncSetAttribute(fa3_attention_fwd, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes);

    // One CTA per q-tile (the kernel does its own batch/head indexing via grid coords).
    dim3 grid(total_rows / Br);
    dim3 block(NTHREADS);
    fa3_attention_fwd<<<grid, block, smem_bytes>>>(
        tmap_Q, tmap_K, tmap_V,
        reinterpret_cast<__half*>(O.data_ptr()),
        seqlen_kv, softmax_scale, is_causal);
    TORCH_CHECK(cudaGetLastError() == cudaSuccess, "flash_attention_fa3_forward launch failed");
    return O;
}
