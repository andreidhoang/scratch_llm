...
// =============================================================================
// HOST LAUNCHER (torch binding) — the runtime glue pybind.cpp binds.
// =============================================================================
// Builds the two CUtensorMap descriptors (A row-major MxK, B col-major KxN —
// the layouts the kernel's TMA loads expect), allocates the fp32 output, and
// launches the 1-SM tcgen05 kernel. One CTA per (M/MMA_M, N/MMA_N) output tile.
//
// Runtime correctness is honestly deferred to the B200 rental day — the kernel
// body is the compile-gated structural spine; this launcher makes it callable.
// =============================================================================
...
torch::Tensor tcgen05_gemm_sm100(torch::Tensor A, torch::Tensor B) {
    TORCH_CHECK(A.is_cuda() && B.is_cuda(), "tcgen05_gemm_sm100: A, B must be CUDA");
    TORCH_CHECK(A.scalar_type() == at::kHalf && B.scalar_type() == at::kHalf,
                "tcgen05_gemm_sm100: A, B must be float16 (tcgen05.f16 operands)");
    TORCH_CHECK(A.dim() == 2 && B.dim() == 2, "tcgen05_gemm_sm100: A, B must be 2D");
    TORCH_CHECK(A.size(1) == B.size(0), "tcgen05_gemm_sm100: K mismatch");
    // tcgen05 1-SM atom is 128x256x16; kernel has no edge handling (rental fast path).
    TORCH_CHECK(A.size(0) % MMA_M == 0 && B.size(1) % MMA_N == 0,
                "tcgen05_gemm_sm100: M must be a multiple of 128, N of 256 (no edge handling)");
    A = A.contiguous();
    B = B.contiguous();   // kernel expects B col-major; we pass row-major B^T via the map
    const int M = A.size(0), K = A.size(1), N = B.size(1);
    auto D = torch::empty({M, N}, A.options().dtype(torch::kFloat32));

    // ---- Build the two CUtensorMap descriptors via the driver API ----
    // A: row-major (M, K), swizzle 128B to match the kernel's SMEM layout.
    // B: the kernel loads B as col-major (K, N); we pass B^T by swapping dims.
    CUtensorMap tma_a = {}, tma_b = {};
    const uint64_t sizeA[2] = {(uint64_t)K, (uint64_t)M};   // (fast, slow) = (K, M) for row-major
    const uint64_t sizeB[2] = {(uint64_t)K, (uint64_t)N};   // B read as (K, N) col-major == B^T row-major
    const uint64_t strideA[1] = {(uint64_t)K * sizeof(__half)};
    const uint64_t strideB[1] = {(uint64_t)K * sizeof(__half)};
    const uint32_t boxA[2] = {MMA_K, MMA_M};
    const uint32_t boxB[2] = {MMA_K, MMA_N};
    const uint32_t elemStrideA[2] = {1, 1};
    const uint32_t elemStrideB[2] = {1, 1};

    CUresult ra = cuTensorMapEncodeTiled(
        &tma_a, CU_TENSOR_MAP_DATA_TYPE_FLOAT16, 2,
        A.data_ptr(), sizeA, strideA, boxA, elemStrideA,
        CU_TENSOR_MAP_INTERLEAVE_NONE,
        CU_TENSOR_MAP_SWIZZLE_128B,
        CU_TENSOR_MAP_L2_PROMOTION_NONE,
        CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
    CUresult rb = cuTensorMapEncodeTiled(
        &tma_b, CU_TENSOR_MAP_DATA_TYPE_FLOAT16, 2,
        B.data_ptr(), sizeB, strideB, boxB, elemStrideB,
        CU_TENSOR_MAP_INTERLEAVE_NONE,
        CU_TENSOR_MAP_SWIZZLE_128B,
        CU_TENSOR_MAP_L2_PROMOTION_NONE,
        CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE);
    TORCH_CHECK(ra == CUDA_SUCCESS && rb == CUDA_SUCCESS,
                "tcgen05_gemm_sm100: cuTensorMapEncodeTiled failed (a=", (int)ra, " b=", (int)rb, ")");

    // SMEM for double-buffered A/B strips + 2 mbarriers + TMEM ptr (kernel-defined extern).
    const int smem_bytes = 2 * (MMA_M * MMA_K + MMA_N * MMA_K) * sizeof(__half) + 256;
    cudaFuncSetAttribute(tcgen05_gemm_1sm, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes);

    dim3 grid(N / MMA_N, M / MMA_M);
    dim3 block(128);   // one warpgroup
    tcgen05_gemm_1sm<<<grid, block, smem_bytes>>>(tma_a, tma_b, D.data_ptr<float>(), K, N);
    TORCH_CHECK(cudaGetLastError() == cudaSuccess, "tcgen05_gemm_sm100 launch failed");
    return D;
}
