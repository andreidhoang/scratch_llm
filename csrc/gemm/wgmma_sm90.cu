...
// =============================================================================
// This is the ONE piece the rental skeleton did not include: the host-side
// `torch::Tensor` wrapper that validates inputs, allocates output, and launches
// the kernel. The kernel body above is unchanged (the compile-gated structural
// spine); this launcher makes it *runnable* from Python on the rental box.
//
// Runtime correctness is STILL honestly gated by the oracle test
// (tests/kernels/test_gemm_wgmma.py, run on H100/H200). The launcher does not
// claim the kernel is correct — it just makes it callable.
// =============================================================================
#ifdef TORCH_EXTENSION_NAME   // only compile when built via setup.py / CMakeLists
#include <torch/extension.h>

torch::Tensor wgmma_gemm_sm90(torch::Tensor A, torch::Tensor B) {
    TORCH_CHECK(A.is_cuda() && B.is_cuda(), "wgmma_gemm_sm90: A, B must be CUDA");
    TORCH_CHECK(A.scalar_type() == at::kHalf && B.scalar_type() == at::kHalf,
                "wgmma_gemm_sm90: A, B must be float16 (wgmma.f16.f16 operands)");
    TORCH_CHECK(A.dim() == 2 && B.dim() == 2, "wgmma_gemm_sm90: A, B must be 2D");
    TORCH_CHECK(A.size(1) == B.size(0), "wgmma_gemm_sm90: K mismatch");
    A = A.contiguous();
    B = B.contiguous();
    const int M = A.size(0), K = A.size(1), N = B.size(1);
    TORCH_CHECK(M % BM == 0 && N % BN == 0,
                "wgmma_gemm_sm90: M,N must be multiples of 64 (no edge handling yet — "
                "the rental measurement sizes are 4096+ so this is the fast path only)");
    auto C = torch::empty({M, N}, A.options().dtype(torch::kFloat32));
    dim3 grid(N / BN, M / BM);
    dim3 block(128);
    wgmma_gemm_sm90a<<<grid, block>>>(
        reinterpret_cast<const half*>(A.data_ptr<at::Half>()),
        reinterpret_cast<const half*>(B.data_ptr<at::Half>()),
        C.data_ptr<float>(),
        M, N, K);
    TORCH_CHECK(cudaGetLastError() == cudaSuccess, "wgmma_gemm_sm90 launch failed");
    return C;
}
