...
| G3 | **Consolidated checkpoint is BUILT (A8 landed 2026-07-19); speedrun launcher does not yet wire it.** `DistMuonAdamW.consolidated_state_dict()`/`load_consolidated()` + `save/load_consolidated_checkpoint()` full-gather the optimizer moments to rank 0 and reshard on resume (bitwise round-trip, incl. cross-topology W2→W1; `tests/test_dist_train.py`). The gap is now only the *launcher*: `speedrun.stage_pretrain` (`speedrun.py:210`) still writes only an end-of-stage *model-only* snapshot and exposes no `--checkpoint-every`, so the consolidated path is reachable **via `train()` directly**, not the speedrun CLI. | **Optimizer-state resume works today via `train()`**; the model weights round-trip regardless (all-gathered to identical replicas every step). Remaining wiring = thread `checkpoint_every` + `save_consolidated_checkpoint` through `speedrun.stage_pretrain` (small, additive). | wire `--checkpoint-every` through `speedrun.stage_pretrain` (the A8 ckpt code itself is done) |
...
1. **Preemption-resumable checkpoints (A8 — ckpt code landed 2026-07-19) `[FACT]`.** A consolidated,
   ...
   warning). **Full optimizer-state resume works today** when the loop is driven via `train()` with
   `checkpoint_every` set; the only remaining wiring is threading `--checkpoint-every` through
   `speedrun.stage_pretrain` (small, additive — do this before launch, or launch via `train()`
   directly). Off-box sync:
   ```bash
