...
> ⚠️ **RE-AIMED 2026-06-27 (read before Part II):** the original open gap — a fused GDN-2 *decode* kernel — was closed by the field (FlashInfer shipped production GDN decode; the MLSys-2026 GDN-decode contest track concluded). The spike now targets **NVFP4 quantization of the GDN recurrent state** (numerics + fused dequant path), per `scratch_llm/docs/STATUS.md` (canonical re-aim note) and `DIRECTION_DECISION_optimlab_vs_cs336.md`. Part I's architecture/roofline analysis remains valid substrate; Part II's kernel target and predictions must be re-derived for the NVFP4 spike before execution. Sequencing unchanged: gated behind the A5 RL ship.
