...
> ⚠️ **DELTA SINCE SYNTHESIS (stamped 2026-07-02):** (1) **Jun 29 → Jul 18 the queue is owned by `Desktop/interview_synthesis/BOOK_SPRINT_2026.md`** — §8's ship-order resumes after the sprint. (2) **DELTA capstone re-aimed** to NVFP4-on-GDN-recurrent-state (see `DELTA.md` banner + `scratch_llm/docs/STATUS.md`); §2/§6 rows referencing "GDN-2 decode = open gap" are stale. (3) Since §5's snapshot: FA2 **backward**, activation checkpointing, mixed-precision lessons, and the **DDP container** shipped (Jun 26–27, `f4e43a8`); ZeRO-1/FSDP still open. (4) `FEINBERG_INTERVIEW_MAP.md` → `_archive/` (FOP extracted into both repos' `CLAUDE.md` §0). Live build state: **`scratch_llm/docs/STATUS.md`**.
