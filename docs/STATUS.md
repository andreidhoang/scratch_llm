...
| `kl_train_infer` bridge (A2.3) — serve vs train | ✅ measured | **real `kl_train_infer` on a 4090**: serve=sdpa/bf16 vs train=eager/{bf16,fp32} on Qwen2.5-0.5B → 0.014 / 0.002 (HALT ok); falsified prediction → **drift = accumulation-precision + kernel, not storage dtype**. `SGLangBackend` written but **sgl_kernel is sm90/sm100-only, no sm89** → SGLang can't run on Ada ([ADR-0008](adr/ADR-0008-sglang-hopper-only-on-ada.md)); HF engine-pair stand-in used. Spec: [`design/L2_kl_train_infer_SPEC.md`](design/L2_kl_train_infer_SPEC.md) |
