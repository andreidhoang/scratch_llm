"""S3 scaling-law sweep CLI — plan / run / fit for the real-corpus IsoFLOP grid (E2E §S3).
    ...
    python scripts/s3_scaling_sweep.py plan [--out PATH]
    python scripts/s3_scaling_sweep.py run --data-dir DATA [--points s1,s2,...]
        [--out-dir DIR] [--batch 32] [--device cuda] [--bf16] [--compile] [--seed 0]
    python scripts/s3_scaling_sweep.py fit [--out-dir DIR] [--results PATH]

``plan`` emits the grid (exact instantiated N, D = ratio × N, C = 6ND per point) without
running anything. ``run`` executes the selected points on the shard-backed pretrain path and
appends each finished record to ``<out-dir>/results.json``. ``fit`` loads results.json, fits
``N_opt ∝ C^a`` / ``D_opt ∝ C^b`` on val_bpb via scaling/isoflop.py, runs the pre-registered
gates (a+b ∈ [0.95, 1.05], log-log R² ≥ 0.98), applies the D:N decision rule, and writes
